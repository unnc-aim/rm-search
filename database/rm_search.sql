...
(
    ...
    `request_length`  int unsigned    NOT NULL DEFAULT 0 COMMENT '请求内容长度',
    ...
    `response_length` int unsigned    NOT NULL DEFAULT 0 COMMENT '响应内容长度',
    `latency`         int unsigned    NOT NULL DEFAULT 0 COMMENT '延迟',
    `create_time`     timestamp(3)    NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    `update_time`     timestamp(3)    NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY `idx_remote_ip` (`remote_ip`),
    KEY `idx_request_length` (`request_length`),
    KEY `idx_query` (`query`),
    KEY `idx_status` (`status`),
    KEY `idx_response_length` (`response_length`),
